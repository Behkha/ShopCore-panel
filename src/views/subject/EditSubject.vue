...
<v-container fill-height fluid grid-list-xl>
  <v-layout justify-center wrap>
    <v-flex xs12 md12>
      <material-card color="green" title="ویرایش موضوع" text="فیلد عنوان موضوع ضروری است">
        <v-form>
          <v-container py-0>
            <v-layout wrap>
              ...
              <v-flex xs12 text-xs-left>
                <v-btn :loading="loading" class="mx-0 font-weight-light" color="success" :disabled="!isSubjectValid" @click="EditSubject()">
                  ویرایش
                </v-btn>
                ...
                <v-dialog v-model="dialog" width="500">
                  <v-card style="direction: rtl">
                    ...
                    <v-card-actions>
                      <v-spacer></v-spacer>
                      <v-btn :loading="loading" color="green darken-1" flat="flat" @click="DeleteSubject()">
                        بله
                      </v-btn>
...
export default {
  ...
  methods: {
    ...
    EditSubject() {
      ...
      this.$store.dispatch('editSubjectRequest', data)
        .then(response => {
          this[l] = false
          this.loader = null
          this.snack_success()
    ...
    DeleteSubject() {
      ...
      const l = this.loader
      this[l] = !this[l]
      this.$store.dispatch('deleteSubjectRequest', {
          id: this.$route.params.id,
        })
        .then(response => {
          this[l] = false
    ...
    fetchSubjectValues() {
      this.$store.dispatch('fetchSubjectValuesRequest', {
          id: this.$route.params.id,
          field: 'contact_us/subjects'
        })
        .then(response => {
          this.subject.title = response.data.data.title
        })
        .catch(error => {
          this.$store.dispatch('handleError', error)
          this.snack_error()
  ...
  mounted() {
    this.subject.title = this.$route.params.title
  },
  computed: {
    isSubjectValid() {
