...
<v-container fill-height fluid grid-list-xl>
  <v-layout justify-center wrap>
    <v-flex xs12 md12>
      <material-card color="green" title="ویرایش کد تخفیف" text="فیلد درصد،  تاریخ شروع و انقضا و حداقل یکی از فیلد های دسته بندی محصول ویژگی ضروری است">
        <v-form>
          <v-container py-0>
            <v-layout wrap>
              ...
              <v-flex xs12 text-xs-left>
                <v-btn :loading="loading" class=" font-weight-light" color="success" :disabled="!isDiscountValid" @click="EditDiscount()">
                  ویرایش
                </v-btn>
                <v-btn :loading="loading" class="mr-2 font-weight-light" color="error" @click="DeleteDiscount()">
                  حذف
                </v-btn>
              </v-flex>
            </v-layout>
...
export default {
  ...
  methods: {
    ...
    DeleteDiscount() {
      ...
      const l = this.loader
      this[l] = !this[l]
      this.$store.dispatch('deleteDiscountRequest', {
          id: this.$route.params.discount.id
        })
        .then(response => {
          this[l] = false
    ...
    setDiscountValues() {
      this.discount.off = this.$route.params.discount.off
      this.discount.expiration_date = this.$route.params.discount.expiration_date
      this.discount.starting_date = this.$route.params.discount.starting_date
    },
    // getCategories() {
  ...
  mounted() {
    ...
    this.setDiscountValues()
    // this.getCategories()
    // this.getProductsList()
  },
  computed: {
    isDiscountValid() {
      return (Boolean(this.discount.off && this.discount.expiration_date && this.discount.starting_date))
    },
    // disableCategory() {
