...
<v-container fill-height fluid grid-list-xl>
  <v-layout justify-center wrap>
    <v-flex xs12 md12>
      <material-card color="green" title="ویرایش برند" text="فیلد عنوان برند ضروری است">
        <v-form>
          <v-container py-0>
            <v-layout wrap>
              ...
              <v-flex xs12 text-xs-left>
                <v-btn :loading="loading" class="mx-0 font-weight-light" color="success" :disabled="!isBrandValid" @click="EditBrand()">
                  ویرایش
                </v-btn>
                ...
                <v-dialog v-model="dialog" width="500">
                  <v-card style="direction: rtl">
                    ...
                    <v-card-text>آیا واقعا میخواهید این برند را حذف کنید ؟</v-card-text>
                    <v-card-actions>
                      <v-spacer></v-spacer>
                      <v-btn :loading="loading" color="green darken-1" flat="flat" @click="DeleteBrand()">
                        بله
                      </v-btn>
...
export default {
  ...
  methods: {
    ...
    EditBrand() {
      ...
      this.$store.dispatch('EditBrandRequest', data)
        .then(response => {
          this[l] = false
          this.loader = null
          this.snack_success()
    ...
    DeleteBrand() {
      ...
      const l = this.loader
      this[l] = !this[l]
      this.$store.dispatch('deleteBrandRequest', {
          id: this.$route.params.id,
        })
        .then(response => {
          this[l] = false
    ...
    fetchBrandValues() {
      this.$store.dispatch('fetchBrandValuesRequest', {
          id: this.$route.params.id,
          field: 'Brands'
        })
        .then(response => {
          this.brand.title = response.data.data.title
        })
        .catch(error => {
          this.$store.dispatch('handleError', error)
          this.snack_error()
  ...
  mounted() {
    this.brand.title = this.$route.params.title
  },
  computed: {
    isBrandValid() {
